...
The `.gitignore` includes `argus-ai/` to prevent automation agents from accidentally cloning the repository inside itself. This is a defense-in-depth measure: if an agent's working directory is inside the repo and it runs `git clone` targeting the same repo, the clone is ignored by Git and will not be tracked or committed. Stray nested clones can be safely deleted with `rm -rf argus-ai/`.
